...
): GraphDataSeries[]
{
    ...
    series_names.forEach((field: Fields[number]) =>
    {
        ...
        const projected_years: { year: number, value: number }[] = []
        all_years.forEach(year =>
        {
            ...
            if (data_point && data_point.value !== undefined)
            {
                ...
                {
                    projected_years.push({ year, value: data_point.value })
                }
            }
        })
        ...
        const proj_points_polyline = proj_years_list.map(dp => `${x_of(dp.year)},${y_of(dp.value)}`).join(" ")

        data_series.push({
            name: field,
            colour: colour_by_series[field],
            points,
            known_points,
